...
InterfaceConfiguration DiffDriveController::command_interface_configuration() const
{
  ...
  for (const auto & joint_name : front_left_wheel_name_)
  {
    conf_names.push_back(joint_name + "/" + HW_IF_VELOCITY);
  }
  for (const auto & joint_name : back_right_wheel_name_)
  {
    conf_names.push_back(joint_name + "/" + HW_IF_VELOCITY);
  }
  for (const auto & joint_name : back_left_wheel_name_)
  {
    conf_names.push_back(joint_name + "/" + HW_IF_VELOCITY);
  }
  for (const auto & joint_name : front_right_wheel_name_)
  {
    conf_names.push_back(joint_name + "/" + HW_IF_VELOCITY);
  }
...
controller_interface::return_type DiffDriveController::update()
{
  ...
  else
  {
    ...
    {
      const double front_left_position = registered_front_left_wheel_handle_[index].position.get().get_value();
      const double front_right_position = registered_front_right_wheel_handle_[index].position.get().get_value();
      const double back_left_position = registered_back_left_wheel_handle_[index].position.get().get_value();
      const double back_right_position = registered_back_right_wheel_handle_[index].position.get().get_value();      
      ...
      {
        RCLCPP_ERROR(
          logger, "Either the left or right wheel positions are invalid for index [%zu]", index);
        return controller_interface::return_type::ERROR;
      }
  ...
  const double velocity_front_left =
    (x - y - z * wheel_separation / 2.0) / left_wheel_radius;
  const double velocity_front_right =
    (x + y + z * wheel_separation / 2.0) / right_wheel_radius;
  ...
  for (size_t index = 0; index < wheels.wheels_per_side; ++index)
  {
    registered_front_left_wheel_handle_[index].velocity.get().set_value(velocity_front_left);
    registered_front_right_wheel_handle_[index].velocity.get().set_value(velocity_front_right);
    registered_back_left_wheel_handle_[index].velocity.get().set_value(velocity_back_left);
    registered_back_right_wheel_handle_[index].velocity.get().set_value(velocity_back_right);
  }
...
CallbackReturn DiffDriveController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  // update parameters
  back_left_wheel_name_ = node_->get_parameter("back_left_wheel_name").as_string_array();
  back_right_wheel_name_ = node_->get_parameter("back_right_wheel_name").as_string_array();
  front_left_wheel_name_ = node_->get_parameter("front_left_wheel_name").as_string_array();
  front_right_wheel_name_ = node_->get_parameter("front_right_wheel_name").as_string_array();

  if (back_left_wheel_name_.size() + front_left_wheel_name_.size()  != front_right_wheel_name_.size() + back_right_wheel_name_.size())
  {
    RCLCPP_ERROR(
      logger, "The number of left wheels [%zu] and the number of right wheels [%zu] are different",
      back_left_wheel_name_.size() + front_left_wheel_name_.size(), front_right_wheel_name_.size() + back_right_wheel_name_.size());
    return CallbackReturn::ERROR;
  }

  if (front_left_wheel_name_.empty() || back_left_wheel_name_.empty())
  {
  ...
  wheel_params_.wheels_per_side = front_left_wheel_name_.size() + back_left_wheel_name_.size();
...
CallbackReturn DiffDriveController::on_activate(const rclcpp_lifecycle::State &)
{
  const auto front_left_result =
    configure_side("front_left", front_left_wheel_name_, registered_front_left_wheel_handle_);
  const auto front_right_result =
    configure_side("front_right", front_right_wheel_name_, registered_front_right_wheel_handle_);
  const auto back_left_result =
    configure_side("back_left", back_left_wheel_name_, registered_back_left_wheel_handle_);
  const auto back_right_result =
    configure_side("back_right", back_right_wheel_name_, registered_back_right_wheel_handle_);
